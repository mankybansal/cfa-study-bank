...
const CALC_TEMPLATES: CalcTemplate[] = [
  {
    ...
    build: (seed) => {
      ...
      return {
        stem: `An investor buys ${shares} shares at $${buy.toFixed(2)}, sells at $${sell.toFixed(2)}, and receives $${dividend.toFixed(2)} dividend per share. Holding period return is closest to:`,
        choices,
        correctIndex,
        explanation: `HPR = (P1 - P0 + D1)/P0 = ${round(hpr, 2).toFixed(2)}%.`,
  ...
  },
  {
    ...
    build: (seed) => {
      ...
      return {
        stem: `A cash flow of $${fv.toLocaleString()} is expected in ${n} years. At ${(r * 100).toFixed(1)}% annual discount rate, present value is closest to:`,
        choices,
        correctIndex,
        explanation: `PV = FV/(1+r)^n = $${round(pv, 2).toFixed(2)}.`,
  ...
  },
  {
    ...
    build: (seed) => {
      ...
      return {
        stem: `Using CAPM: Rf ${(rf * 100).toFixed(1)}%, market return ${(rm * 100).toFixed(1)}%, beta ${beta.toFixed(2)}. Required return is closest to:`,
        choices,
        correctIndex,
        explanation: `E(R) = Rf + beta[Rm-Rf] = ${round(required, 2).toFixed(2)}%.`,
  ...
  },
  {
    ...
    build: (seed) => {
      ...
      return {
        stem: `Bond modified duration ${md.toFixed(2)}, convexity ${cx.toFixed(2)}. If yield rises ${(dy * 10000).toFixed(0)} bps, estimated % price change is closest to:`,
        choices,
        correctIndex,
        explanation: `DeltaP/P ≈ -D*DeltaY + 0.5*C*(DeltaY)^2 = ${round(pct, 2).toFixed(2)}%.`,
  ...
  },
  {
    ...
    build: (seed) => {
      ...
      return {
        stem: `A regression line shown on a chart has intercept ${intercept.toFixed(2)} and slope ${slope.toFixed(2)}. At x=${x.toFixed(0)}, fitted y-value is closest to:`,
        choices,
        correctIndex,
        explanation: `From line equation y = a + bx, y = ${round(y, 2).toFixed(2)}.`,
